...
export class AuthService {
	...
	async signup(dto: SignupAuthDto) {
		...
		return {accessToken, refreshToken,userId: newUser.id, role: newUser.role.name}
	}

	async signin(dto: SigninAuthDto) {
		const user = await this.userService.findByEmail(dto.email)
		if (!user) throw new UnauthorizedException("не правильный email или пароль")
		const isEqualPasswords = await bcrypt.compare(dto.password, user.password)
		if (!isEqualPasswords) throw new UnauthorizedException("не правильный email или пароль")
	...
	async refresh(refreshToken: string) {
		...
		return {accessToken, refreshToken: newRefreshToken, userId: user.id, role: user.role.name}
	}
